...
public class ScriptEditScreen extends GuiConfigsBase {
	...
	@Override public void drawTitle(GuiGraphics context, int mouseX, int mouseY, float partialTicks) {
		adjustPosition();
		//渲染内容
		var root = getRootDisplayWidget();
		...
		if(scriptHoldingData != null){
			...
			double dx = method.moveX ? mouse.xpos() - holdingPos.x - x - hoverWidget.getX() : 0;
			double dy = method.moveY ? mouse.ypos() - holdingPos.y - y - hoverWidget.getY() : 0;
	...
	public void setHover(WidgetBase widget, int mouseX, int mouseY, PoseStack.Pose entry){
		hoveredRenderer.setHover(widget, mouseX, mouseY, entry);
	}
	
	public void setHover(HoveredClickableWidget widget, int mouseX, int mouseY, PoseStack.Pose entry){
		hoveredRenderer.setHover(widget, mouseX, mouseY, entry);
	}
	
	public void renderAndTryHover(GuiGraphics context, ButtonGeneric buttonGeneric, int fixedMouseX, int fixedMouseY){
		...
		if(isOver) setHover(buttonGeneric, fixedMouseX, fixedMouseY, context.pose().last());
	}
	
	@Override protected void drawHoveredWidget(int mouseX, int mouseY, GuiGraphics context) {
	...
	private static class HoveredRenderer{
		...
		private final PoseStack renderMatrix = new PoseStack();
		...
		public void tryRender(GuiGraphics context){
			if(hoveredWidget != null){
				var matrices = context.pose();
				matrices.pushPose();
				DataUtils.copy(matrices.last(), renderMatrix.last());
		...
		public void setHover(WidgetBase widget, int mouseX, int mouseY, PoseStack.Pose entry){
			...
			storedMouseX = mouseX;
			storedMouseY = mouseY;
			DataUtils.copy(renderMatrix.last(), entry);
		}
		public void setHover(HoveredClickableWidget widget, int mouseX, int mouseY, PoseStack.Pose entry){
