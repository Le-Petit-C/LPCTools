...
public enum RenderTiming implements IUnregistrableRegistry<IRenderCallback> {
	START_MAIN(Registries.START, callback-> _ ->callback.render()),
	AFTER_SETUP(Registries.AFTER_SETUP, callback->_->callback.render()),
	BEFORE_ENTITIES(Registries.BEFORE_ENTITIES, callback->_->callback.render()),
	AFTER_ENTITIES(Registries.AFTER_ENTITIES, callback->_->callback.render()),
	BEFORE_BLOCK_OUTLINE(Registries.BEFORE_BLOCK_OUTLINE, callback->(_, _)->{callback.render(); return true;}),
	BLOCK_OUTLINE(Registries.BLOCK_OUTLINE, callback->(_, _)->{callback.render(); return true;}),
	BEFORE_DEBUG_RENDER(Registries.BEFORE_DEBUG_RENDER, callback->_->callback.render()),
	AFTER_TRANSLUCENT(Registries.AFTER_TRANSLUCENT, callback->_->callback.render()),
	ON_LAST(Registries.ON_LAST, callback->_->callback.render()),
	ON_END(Registries.ON_END, callback->_->callback.render());
