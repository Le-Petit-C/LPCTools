...
public class VanillaBlockInteractionModifier {
    ...
    private static boolean quietBlockBreakHotkeyCallback(KeyAction keyAction, IKeybind iKeybind) {
        ...
        if(!player.isCreative() || !player.hasPermissionLevel(2)
            || !(client.crosshairTarget instanceof BlockHitResult hitResult))
            return normalAttack();
        LinkedHashMap<BlockPos, BlockState> brokenStates = buildBrokenBlockPoses(hitResult.getBlockPos(), player.getWorld());
        while (!brokenStates.isEmpty()) {
            ...
            player.getWorld().playSoundAtBlockCenter(
            ...
            player.getWorld().addBlockBreakParticles(pos, state);
