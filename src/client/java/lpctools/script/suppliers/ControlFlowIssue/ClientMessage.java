...
public class ClientMessage extends AbstractSupplierWithTypeDeterminedSubSuppliers implements IControlFlowIssueSupplier {
	protected final SupplierStorage<String> message = ofStorage(String.class,
		Component.translatable("lpctools.script.suppliers.controlFlowIssue.clientMessage.subSuppliers.message.name"), "message");
	protected final SupplierStorage<Boolean> overlay = ofStorage(Boolean.class,
		Component.translatable("lpctools.script.suppliers.controlFlowIssue.clientMessage.subSuppliers.overlay.name"), "overlay");
	...
	compileNotNull(CompileEnvironment environment) {
		...
		return map->{
			clientMessage(Component.literal(compiledStringSupplier.scriptApply(map)), compiledBooleanSupplier.scriptApply(map));
