...
public class ConstantType extends AbstractScript implements ITypeSupplier {
	...
	protected ScriptType cachedType = ScriptType.getType(Object.class);
	...
	@Override public @Nullable Iterable<?> getWidgets() {
		if(cachedWidgets == null){
			var selectButton = new WidthAutoAdjustButtonGeneric(getDisplayWidget(), 0, 0, 20, cachedType.name().getString(), null);
			selectButton.setActionListener((button, mouseButton)->{
				...
				ChooseScreen.openChooseScreen(Text.translatable("lpctools.script.suppliers.types.constantType.selectType.title").getString(),
	...
	@Override public @Nullable JsonElement getAsJsonElement() {
		return new JsonPrimitive(cachedType.id());
	}
	@Override public void setValueFromJsonElement(@Nullable JsonElement element) {
		...
		if (element instanceof JsonPrimitive primitive) {
			...
			if(ScriptSupplierLake.typeIdMap.containsKey(id)){
				cachedType = ScriptSupplierLake.typeIdMap.get(id);
				return;
			}
		}
